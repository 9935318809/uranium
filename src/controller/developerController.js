...
const getDeveloperByPP = async (req,res)=> {
    ...
    const getData = await developerModel.find({percentage:{$gte:parseInt(req.query.percentage)},batch:{$in:batch_id}}).populate('batch_id',{name:1,program:1,_id:0})
...
const getAllDevelopers = async (req,res) => {
    const getData = await developerModel.find().populate('batch_id',{name:1,program:1,_id:0})
    res.send({msg:getData})
}
